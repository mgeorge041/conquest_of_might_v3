...
public class GamePieceObject : MonoBehaviour
{
    ...
    public static Unit LoadTestUnit() {
        CardUnit cardUnit = (CardUnit)Card.LoadTestUnitCard();
        Unit newUnit = new Unit(cardUnit, 0);
        newUnit.SetCard((CardUnit)Card.LoadTestUnitCard());
        newUnit.SetInfo();
        newUnit.ResetPiece();
        return newUnit;
    }
    ...
    public static GamePieceObject InitializeFromCardPiece(CardPiece cardPiece, Transform parentTransform, int playerId) {
